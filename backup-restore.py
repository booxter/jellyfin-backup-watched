...
def backup(username=None):
    usersurl = f"{jellyfin_url}/Users"
    users_resp = requests.get(usersurl, params={"apikey": apikey})
    ...
    for user in users:
        ...
        for item in data_favorited_people['Items']:
            ...
            user_output["items"].append(outputitem)
        ...
        for item in data['Items']:
            ...
            if item['LocationType'] == "Virtual":
                continue
            pathrequest = requests.get(
                f"{jellyfin_url}/Users/{userid}/Items/{itemid}",
        ...
        output["users"].append(user_output)
...
def restore(dryrun=False):
    ...
    # Support legacy single-user format by wrapping it.
    users_data = data.get("users")
    if not users_data:
        users_data = [{"username": data.get("username"), "items": data.get("items", [])}]

    for user_entry in users_data:
        username = user_entry.get("username")
        items = user_entry.get("items", [])
        print(f"Restoring {len(items)} items for {username} from {data.get('backupdate')}...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--username", help="Jellyfin user to back up (omit to back up all users)")
